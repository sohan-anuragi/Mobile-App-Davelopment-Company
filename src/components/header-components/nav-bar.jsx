...
import NavHeadings from './nav-headings'
import DropBoxDataProvider from './headerstore'
import DropBox from './dropbox'
import NavHeadingsResponsive from './nav-headings-responsive'
  ...
  const handleMenu = () => {
    ...
    return <>
     <div href='#' className={`${styles['navbar-container']}`}>
      ...
      onClick={handleMenu}
      >
      {toggleMenu === false ? <IoMenu className={styles['menu-icon']}></IoMenu>: <RxCross2 className={styles['menu-icon']}></RxCross2>}
      </div>
    </div>
